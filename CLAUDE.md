...
- **이의(dispute) 제거됨** — 확인 전용. route는 confirm만 통과, 그러나 **뷰의 `confirm_result='dispute'` case·트레이너 이의 뱃지·pending 필터의 `!=='dispute'`는 죽은 분기로 잔존**(무해 · **정리 대상**: 일괄 제거 예정). dispute 행은 더는 안 생김.
