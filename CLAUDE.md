...
- `app/admin/page.jsx` — **admin dashboard**(원장 전용). 섹션 탭 `ATABS`(6): **실적**(실데이터 요약 KPI + `TrainerScorecard` 리더보드 + 클로징/재등록 분포)·**전환**(`ConversionFunnel`)·**리텐션**(`RetentionConsole`)·**QC**(데모)·**급여**(`AdminPayrollSettings`)·**운영**(트레이너 초대·공지·카피봇). ⚠️ 구 "트레이너별 실적" 인라인 카드는 **`TrainerScorecard`로 대체**(제거). 분석 3종은 `components/admin/`(아래 "admin 분석 대시보드").
...
### admin 분석 대시보드 — 리더보드·리텐션·퍼널 (v2 · 2026-07-25)

원장용 실적/관제 3화면. **전부 실데이터**(QC 탭 데모와 다름) · **admin이 이미 로드한 배열(`rows`=user_table·`otRows`·`contracts`=session_log·`logs`=daily_workout_log·`trainers`·`schemes`·`runs`)을 props로 받아 파생만** — DB 쿼리 추가 0 · 마이그레이션 0 · RLS 무변. 스펙: `docs/v2-스펙-트레이너KPI스코어카드-리더보드-구현.md`·`v2-스펙-재등록이탈관제-구현.md`·`v2-스펙-전환퍼널-OT-PT-구현.md`. 핸드오프: `docs/v2-핸드오프-2026-07-25-admin분석대시보드.md`.

- **컴포넌트(`components/admin/`):** `TrainerScorecard`(실적 탭 리더보드 · 트레이너 한 줄 KPI + 펼침 `PayrollConfirm`)·`RetentionConsole`(리텐션 탭 · 만료임박·이탈위험·재등록률)·`ConversionFunnel`(전환 탭 · OT→PT 깔때기·트레이너 약점·이번주 클로징 임박).
- **파생 함수(`lib/memberStatus.js` · 전부 순수·기준시각 주입):**
  ...
  - 퍼널: `otFunnel`·`otFunnelByTrainer`(라운드별 `firstSuccess`/`secondSuccess` 포함)·`closingDueSoon`(+내부 `_otResultByMember`).
