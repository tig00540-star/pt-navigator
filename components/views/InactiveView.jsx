...
export default function InactiveView({ member, onGoList }) {
  return (
    <section className="rounded-2xl border border-dashed border-line bg-card p-6 shadow-sm">
      {onGoList && (
        <button
          onClick={onGoList}
          className="mb-3 inline-flex items-center gap-1.5 text-xs font-medium text-sub transition hover:text-primary-strong"
        >
      ...
      {member.status_note && (
        <p className="mt-2 text-sm text-sub">
          사유: <span className="text-sub">{member.status_note}</span>
        </p>
      )}
