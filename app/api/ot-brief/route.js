...
진입→그림→착지→침묵. 등록은 목표가 아니라 회원이 깊이 이해받았을 때 따라오는 결과다.
 ...
 - paint(그림): 회원 직업·일상에서 끌어온 '일상 비유 하나'로 머릿속에 그림을 그린다
   (운동/기계 클리셰 "브레이크-액셀"·"기름칠" 금지 — 회원 세계의 언어로). 2문장 이내.
 - land(착지): '왜 필요 + 왜 지금'. 판매 동사('등록하세요') 금지 → 가정 종결
...
decider(배우자·가족과 상의) / doubt(효과·필요성 의심). 각 항목:
 ...
 - sales_move: 그 다음 세일즈 방향 — 무엇을 보여주고 어디로 이끄나. 1문장.
 - example: 예시 한 문장(발판). 1문장.
...
(공감>압박, 숫자·URL 금지 등)은 그대로 두고 '길이만' 압축한다(필드를 비우거나 빼지는 말 것).
 ...
 - objections: 각 필드(customer_says·reframe_direction·sales_move·example) 1문장(발판이라 짧게).
