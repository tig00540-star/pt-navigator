...
### [미래] 트레이너 프로필 기반 세일즈 개인화
- 트레이너 성향·MBTI·세일즈 방향·자신있는 운동 축(재활/웨이트/기능성 등)을 입력 → 1·2차·재등록
  브리핑을 트레이너 결에 맞춤. 범용 대본이 아니라 "이 트레이너가 잘하고 편한 방식"으로 길잡이(상향평준화).
- 파이프라인 = sales_intensity 주입 패턴의 확장(회원별이 아니라 트레이너 1명당 고정 재료).
- ⚠️ 선행 = ⑦(trainer 테이블·소유권) — 트레이너가 누구인지 앱이 알아야 저장 가능(현재 anon).
  단일 트레이너 가정이면 로그인 전 우회로(프로필 1벌 로컬/설정 저장 → route 주입) 가능(자리 미리·값 나중).
- ⚠️ 가드레일: 트레이너 "세일즈 방향"이 압박으로 새지 않게 — b의 sales_intensity strong 가드와 동일 방어
  (성향은 톤·강조축을 바꾸지 PREAMBLE 윤리를 못 넘음).
- ⑤(자신있는 운동 축 반영)와 짝.
