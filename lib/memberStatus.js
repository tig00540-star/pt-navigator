...
/**
 * 트레이너별 퍼널 — 어디서 새는지 비교용.
 * @returns {Array<{trainer_id, intake, first, second, confirmed, convRate}>} 유입 내림차순.
 */
export function otFunnelByTrainer(members, otRows) {
  const subjects = (Array.isArray(members) ? members : []).filter(isClosingStatSubject);
  const res = _otResultByMember(otRows);
  const map = new Map();
  const get = (tid) => { let c = map.get(tid); if (!c) { c = { intake: 0, first: 0, second: 0, confirmed: 0 }; map.set(tid, c); } return c; };
  for (const m of subjects) {
