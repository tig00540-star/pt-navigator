...
/**
 * 이번 달 트레이너별 OT 진행 수업 수 — ot_log 기준(created_at 해당월 KST · round 1·2 각 1건).
 * PT는 sessionsThisMonthByTrainer(session_at)로 별도. 트레이너 귀속 = 회원 현재 담당(memberTrainer).
 * @param {Array} otRows  ot_log 행(user_id·ot_round·created_at)
 * @param {Map} memberTrainer  Map(user_id → trainer_id)
 * @param {string} ym  'YYYY-MM'(KST)
 * @returns {Map<string, number>} trainer_id → OT 진행 수(미배정='unknown')
 */
export function otSessionsThisMonthByTrainer(otRows, memberTrainer, ym) {
  const mt = memberTrainer instanceof Map ? memberTrainer : new Map();
  const map = new Map();
  for (const r of Array.isArray(otRows) ? otRows : []) {
    if (!r || r.user_id == null) continue;
    if (r.ot_round !== 1 && r.ot_round !== 2) continue;
    if (typeof r.created_at !== "string" || kstYm(r.created_at) !== ym) continue;
    const tid = mt.get(r.user_id) ?? "unknown";
    map.set(tid, (map.get(tid) || 0) + 1);
  }
  return map;
...
/* ── 이탈/만료 파생 공통 헬퍼(모듈 내부 · churnRiskByTrainer·churnRiskMembers·expiringMembers 공유) ── */

// 기준시각(nowISO) 주입 → daysSince(iso)=경과일(floor · 순수 ms 차). iso 없거나 파싱실패면 null.
...
export function churnRiskByTrainer(members, contracts, logs, { nowISO, staleDays = 14 } = {}) {
  const daysSince = makeDaysSince(nowISO);
  const map = new Map();
  const get = (tid) => {
    let c = map.get(tid);
  ...
  // 회원별 계약·로그 버킷(O(n) 1회 구축 · 중첩 스캔 회피). 변수명 보존(별칭 구조분해).
  const { cBy: cByMember, lBy: lByMember } = bucketByMember(contracts, logs);
...
export function churnRiskMembers(members, contracts, logs, { nowISO, staleDays = 14 } = {}) {
  const daysSince = makeDaysSince(nowISO);
  const { cBy, lBy } = bucketByMember(contracts, logs);
  const out = [];
  for (const m of Array.isArray(members) ? members : []) {
    if (!m || viewFor(m) !== "pt") continue;
