...
export function revenueByTrainer(contractRows, ym) {
  ...
  const map = new Map();
  for (const r of rows) {
    if (!r || !r.counts_as_revenue) continue;
    if (typeof r.started_at !== "string" || kstYm(r.started_at) !== ym) continue;
    const tid = r.trainer_id ?? "unknown";
    const cur = map.get(tid) || { newRev: 0, reRev: 0, total: 0, cntNew: 0, cntRe: 0 };
